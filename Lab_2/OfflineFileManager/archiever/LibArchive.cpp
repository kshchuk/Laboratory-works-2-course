...
inline static QString dirName( QString path ) {
	...
	QString dirPth = QString(QFileInfo(dupPath).dir().dirName()) + "/";
	free( dupPath );
...
inline static QString baseName( QString path ) {
	...
	QString basePth = QString(QFileInfo(dupPath).baseName());
	free( dupPath );
...
inline static int mkpath( QString path) {
	...
	return mkdir( path.toLocal8Bit().constData());
};
